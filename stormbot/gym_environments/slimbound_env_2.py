...
# Action space:
# 2 Card plays (1, 2 or 3) on any position on the board
# size(action_space) = card1, card2, pos1, pos2 = 3 * 2 * 4 * 3 * 2

class SlimboundEnv_2(gym.Env):
    ...
    def __init__(self, seed = None, **kwargs) -> None:
        ...
        self.action_space = gym.spaces.Discrete(3 * 2 * ROWS * COLS * 2 + 1)
    ...
    def _execute_action(self, action: int):
        self._start_turn()
        if action != 0:
            card1, row1, col1, card2, row2, col2 = decode_unit_action(action)
            self._play_unit_card(card1, row1, col1)
            self._play_unit_card(card2, row2, col2)
        self._end_turn()
...
'''
    ...
    def legal_actions(self):
        ...
        if avaliable_positions:
            single_action = [(card, pos) for card in cards for pos in avaliable_positions]
            actions = [
                encode_unit_action(card1, pos1, card2, pos2)
                for card1, pos1 in single_action
                for card2, pos2 in single_action
                if card1 != card2
            ]
...
def encode_unit_action(card1, pos1, card2, pos2) -> int:
    return np.ravel_multi_index((card1, *pos1, card2, *pos2), (HAND_SIZE, ROWS, COLS, HAND_SIZE, ROWS, COLS)) + 1

def decode_unit_action(action: int) -> tuple:
    return np.unravel_index(action-1, (HAND_SIZE, ROWS, COLS, HAND_SIZE, ROWS, COLS))

def action_to_str(action_number):
        if action_number == 0:
            action = 'End Turn'
        else:
            idx1, row1, col1, idx2, row2, col2 = decode_unit_action(action_number)
