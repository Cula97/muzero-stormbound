...
class Game(AbstractGame):
    ...
    def action_to_string(self, action_number):
        ...
        """
        row, col = divmod(action_number, 3)
...
class TicTacToe_V2:
    ...
    def get_observation(self):
        ...
        return np.array([board_player, board_enemy], dtype=np.int8)

    def legal_actions(self):
        return [i for i in range(9) if not self.board[divmod(i, 3)]]

    def have_winner(self):
        # Horizontal and vertical checks
        for i in range(3):
            if (self.board[i, :] == self.player * np.ones(3, dtype=np.int8)).all():
                return True
            if (self.board[:, i] == self.player * np.ones(3, dtype=np.int8)).all():
                return True
