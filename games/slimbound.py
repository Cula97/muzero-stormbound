...
import gym
import numpy as np
from stormbot.gym_environments import slimbound_env
import torch
...
class MuZeroConfig:
    def __init__(self):
        ...
        self.action_space = list(range(3*4*3 +1))  # Fixed list of all possible actions. You should only edit the length
...
class Game(AbstractGame):
    ...
    def __init__(self, seed=None):
        self.env = gym.make("SlimboundEnv")
        if seed is not None:
            self.env.seed = seed
    ...
    def step(self, action):
        ...
        return self._transpose(observation), reward * 10, done
    ...
    def action_to_string(self, action_number):
        ...
        if action_number == 0:
            ...
            card, row, col = slimbound_env.decode_unit_action(action_number)
            action = f'Play card {card+1} at [{row}, {col}]'
